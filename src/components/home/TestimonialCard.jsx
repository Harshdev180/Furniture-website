import React from "react";

const TestimonialCard = ({ name, role, img, review, rating }) => {
  const fullStars = Math.round(Number(rating) || 0);
  return (
    <div className="text-sm w-70 mx-auto border border-gray-200 rounded-lg bg-[#FAF7F2] shadow-xl shadow-black/30 py-10">
      <div className="flex flex-col items-center px-5 py-4">
        <img className="h-24 w-24 rounded-full shrink-0" src={img} alt={name} />
        <div className="pt-6 text-center">
          <h1 className="text-lg font-medium text-[#3E2723] font-serif">
            {name}
          </h1>
          <p className="text-gray-800/80">{role}</p>
        </div>
      </div>
      <p className="text-gray-600 px-6 text-center">{review}</p>
      <div className="flex justify-center pt-3 pb-4">
        <div className="flex gap-0.5 pt-10">
          {[...Array(5)].map((_, index) => (
            <svg
              key={index}
              width="18"
              height="18"
              viewBox="0 0 22 20"
              fill={index < fullStars ? "#FF532E" : "#E6D5C3"}
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M10.525.464a.5.5 0 0 1 .95 0l2.107 6.482a.5.5 0 0 0 .475.346h6.817a.5.5 0 0 1 .294.904l-5.515 4.007a.5.5 0 0 0-.181.559l2.106 6.483a.5.5 0 0 1-.77.559l-5.514-4.007a.5.5 0 0 0-.588 0l-5.514 4.007a.5.5 0 0 1-.77-.56l2.106-6.482a.5.5 0 0 0-.181-.56L.832 8.197a.5.5 0 0 1 .294-.904h6.817a.5.5 0 0 0 .475-.346z" />
            </svg>
          ))}
        </div>
      </div>
    </div>
  );
};

export default TestimonialCard;
